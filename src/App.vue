<template>
    <main class="posts">
        <h2 class="posts__title">Posts</h2>
        <ul class="posts__posts-content">
            <template v-for="(post,i) in posts" v-bind:key="post.id">
                <li v-if="i < showCount" class="content__postItem">
                    <div class="postItem__main-part">
                        <img class="postItem__img" :src="post.img" alt="post.title">
                        <div class="postItem__text">
                            <h3>{{post.title}}</h3>
                            <p>{{post.description}}</p>
                            <a href="">подробнее</a>
                        </div>
                    </div>
                    <div class="postItem__btn">
                        <button>Нравиться</button>
                    </div>
                </li>
            </template>
            <div v-if="showCount < posts.length" class="postItem__btn">
                <button @click="showMore">Смотреть больше...</button>
            </div>
        </ul>
    </main>
</template>


<script>
    import PostItem from "./components/PostItem";

    export default {
        components: {PostItem},
        data() {
            return {
                showCount: 3,
                posts: [
                    {
                        id: 1, title: 'Vue',
                        img: "data:image/png;base64,",
                        description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Beatae, cumque cupidita'
                    },
                    {
                        id: 2, title: 'React',
                        img: "data:image/png;base64,",
                        description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Beatae, cumque cupidita'
                    },
                    {
                        id: 3, title: 'Angular',
                        img: "data:image/png;base64,",
                        description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Beatae, cumque cupidita'
                    },
                    {
                        id: 4, title: 'Vue',
                        img: "data:image/png;base64,",
                        description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Beatae, cumque cupidita'
                    },
                    {
                        id: 5, title: 'React',
                        img: "data:image/png;base64,",
                        description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Beatae, cumque cupidita'
                    },
                    {
                        id: 6, title: 'Angular',
                        img: "data:image/png;base64,",
                        description: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Beatae, cumque cupidita'
                    },
                ]
            }
        },
        methods: {
            showMore() {
                this.showCount += 3
            }
        }
    }

</script>


<style>
    * {
        margin: 0;
        padding: 0;
        box-sizing: border-box;
    }

    body {
        font: 16px sans-serif;
        margin: 0;
        padding: 0;
    }

    .posts {
        padding: 20px;
        max-width: 1200px;
        margin: 0 auto;
    }

    .posts .posts__title {
        text-align: center;
        margin-bottom: 15px;
    }

    .posts__posts-content {
        width: 100%;
        list-style: none;
    }

    .posts .content__postItem {
        padding: 15px;
        margin-bottom: 20px;
        display: flex;
        justify-content: space-between;
        border: 2px solid #3e8969;
        align-items: center;
    }

    .postItem__img {
        max-width: 150px;
        margin-right: 15px;
    }

    .postItem__main-part {
        max-width: 40%;
        display: flex;
        align-items: center;
    }

    .postItem__main-part p {
        margin: 6px 0;
    }

    .postItem__main-part a {
        color: #3e8969;
        text-decoration: none;
    }

    .postItem__btn button {
        display: block;
        background: #3e8969;
        padding: 4px 7px;
        cursor: pointer;
        border: none;
        color: #fff;
        border-radius: 3px;
    }
</style>
